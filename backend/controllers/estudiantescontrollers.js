...
class EstudiantesController {
      ...
      db.query(
        ...
        (err, rows) => {
          ...
          if (rows.affectedRows == 1)
            res
              .status(200)
              .json({ respuesta: "Registro actualizado correctamente" });
        }
      );
    } catch (err) {
      //console.log(err);
  ...
  ingresar(req, res) {
    try {
      ...
      db.query(
        "INSERT INTO estudiantes (id, dni, nombre, apellidos, email) VALUES (NULL, ?, ?, ?, ?);",
  ...
  borrar(req, res) {
    ...
    try {
      req.body;
      db.query(
        "DELETE FROM taller2db.estudiantes WHERE id=?;",
        [id],
        (err, rows) => {
          if (err) {
            res.status(400).send(err.message);
          ...
          if (rows.affectedRows == 1)
            res
              .status(200)
              .json({ respuesta: "Registro borrado correctamente" });
        }
      );
    } catch (err) {
      res.status(500).send(err.message);
