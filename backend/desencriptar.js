...
function desencriptarmensaje(mensaje){
    ...
    importKeyFromString(key).then(function(keyP){
        const hola = decryptAES(mensaje,keyP).then(function(texto){
            ...
            importPrivateKey(Prkey,2048).then(function(llave){
                decryptDataWithPrivateKey(texto,llave).then(function(mensaje){
                    return message = arrayBufferToString(mensaje)
                })
            })
        })
    })
...
function decryptAES(data,key) {
    ...
    return crypto.subtle.decrypt(
        ...
        data
            ...
            let codigo = mensajeEnc.substring(mensajeEnc.length - 3, mensajeEnc.length)
            mensajeEnc= mensajeEnc.substring(0, mensajeEnc.length - 3)
            return mensajeEnc,codigo
        });
}
